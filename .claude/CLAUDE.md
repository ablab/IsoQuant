...
See `.claude/BARCODE_GRAPH_CORRECTION.md` for graph-based barcode correction (branch `badger`, ported from https://github.com/algbio/Badger) — the alternative to per-read whitelist matching for stock 10x whitelists, where `min_score == BARCODE_LEN_10X` makes matching degenerate into exact matching. Benchmarked on three real 10x CI datasets (ONT StereoQ, ONT cDNA R10.4, concat/split): against a stock 7.4M whitelist graph correction gains ~12 points of recall and some precision, and its result is *identical* to using the matched small whitelist — the whitelist only filters candidate centers, so its size stops mattering. With a correctly sized whitelist it costs ~1 point of recall, hence the `auto` default. Extract barcode windows verbatim (`TenXBarcodeDetector(whitelist_matching=False)`), count them over the run, select cluster centers from the count distribution ∩ whitelist, and correct the rest by edit distance. Selected via `--barcode_correction {auto,whitelist,graph}` (auto → graph above 100k whitelist barcodes) plus `--n_cells`. Architecture: `BarcodeGraph` (`isoquant_lib/barcode_calling/barcode_graph.py`) + the `correct_barcodes` stage (`correct_barcodes.py`), writing `aux/<prefix>.raw_barcodes_<i>.tsv` → `<prefix>.barcoded_reads_<i>.tsv`. Two provably equivalent implementations, `--barcode_graph_impl {centers,full}`; `centers` is the default and 4.9x faster. Also available from the standalone `isoquant_detect_barcodes.py`. Three gotchas documented there, all found by measurement: strand selection must be structural (R1-to-polyT span), because raw mode has no match score to pick an orientation with — worth 20 points of recall; `hits_delta` counts k-mer *occurrences*, so it must be set to `(L-k+1)²` to actually disable hit pruning; and `optimal_kmer_size` picks the largest k the q-gram bound allows (k=8 for 16bp at threshold 1, not Badger's 6), which is 9.1x faster clustering for identical output. Round 2 of clustering is the remaining bottleneck — ~2/3 of runtime for +1.3 points of recall; `--barcode_graph_rounds 1` trades it away.
