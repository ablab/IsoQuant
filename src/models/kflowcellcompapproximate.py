...
class kFlowCellTypeDecomp(pathmodel.AbstractPathModelDAG):
    ...
    def _encode_flow_decomposition(self):
        ...
        for u, v, data in self.G.edges(data=True):
            ...
            for cell_type in self.cell_types:
                ...
                # lowerbound
                self.solver.add_constraint(
                    self.solver.quicksum(self.pi_vars[(u, v, i, cell_type)] for i in range(self.k)) >= lamb0 * int(ct_u_v[self.cell_tree.get_cell_type_index(cell_type)]),
                    name=f"tcf_u={u}_v={v}_ct={cell_type}"
                )

                # upperbound
                self.solver.add_constraint(
                    self.solver.quicksum(self.pi_vars[(u, v, i, cell_type)] for i in range(self.k)) <= lamb1 * int(ct_u_v[self.cell_tree.get_cell_type_index(cell_type)]),
                    name=f"tcf_u={u}_v={v}_ct={cell_type}"
                )
    ...
    def _encode_given_weights(self):
        ...
        self.solver.set_objective(
            self.solver.quicksum(self.path_weights_vars[(i, ct)] for i in self.path_indexes for ct in self.cell_types),
            sense = "maximize",
        )
