    ...
    def _encode_flow(self):
        ...
        for u, v, data in self.G.edges(data=True):
            ...
            for ct in self.cell_types:
                ...
                f_u_v_ct = f_u_v[self.cell_tree.get_cell_type_index(ct)]

                self.solver.add_constraint(
                    f_u_v_ct - self.edge_vars[(u, v, ct)] <= self.edge_error_vars[(u, v, ct)],
                    name=f"edge_error_u={u}_v={v}",
                )

                self.solver.add_constraint(
                    self.edge_vars[(u, v, ct)] - f_u_v_ct <= self.edge_error_vars[(u, v, ct)],
                    name=f"edge_error_u={u}_v={v}",
                )
