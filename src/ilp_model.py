...
def clean_graph(G,additional_starts,additional_ends):
    nodes_to_remove = []
    for node in G.nodes():
...
def Intron2Nx_Node(
    ...
    if skip_isolated_nodes:
        clean_graph(G,additional_starts,additional_ends)
...
def ILP_Solver_Nodes(intron_graph,chr_id, gene_id,constraints=[] ,ground_truth_isoforms=[], epsilon=0.25, timeout=300, threads=5):
    ...
    if not(len(graph.nodes()) == 0 or len(graph.edges())== 0): #the graph is not empty (contains nodes and edges)
        if len(graph.edges()) < 500: #we have a reasonably sized graph (not too big for the model)
            ...
            res = list(zip(original_paths, weights))
    else: #we either have no edges or nodes so we do not return a result
        res = []
