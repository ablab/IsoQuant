...
def convert_to_matrix(input_linear_counts, output_file_path, feature_id_to_name_dict=None, gzipped=False):
    ...
    df = pandas.read_csv(input_linear_counts, delimiter='\t', header=None, skiprows=1, names=['gene_id', 'group_id', 'count'])
    ...
    with gzip.open(output_file_path, 'wt') if gzipped else open(output_file_path, 'w') as outfile:
        ...
        outfile.write('gene_id\t' + '\t'.join(columns) + '\n')
        ...
        for gene_id, row in count_matrix.iterrows():
            ...
            values = [str(row[col]) if pandas.notna(row[col]) else '0' for col in columns]
...
def convert_to_mtx(input_linear_counts, output_file_prefix, feature_id_to_name=None, gzipped=False):
...
def parse_args():
    ...
    parser.add_argument("--gzip", help="gzip output files", dest="gzipped", action='store_true', default=False)
