...
class Array2BitKmerIndexer:
    ...
    def _get_kmer_bin_indexes(self, bin_seq, seq_len):
        for i in range(seq_len - self.k + 1):
            yield (bin_seq >> ((seq_len - self.k - i) * 2)) & self.mask

    def _get_kmer_bin_indexes_bc(self, bin_seq):
        return self._get_kmer_bin_indexes(bin_seq, self.seq_len)

    def _index(self, known_bin_seq):
        for bin_seq in known_bin_seq:
            self.total_sequences += 1
            for kmer_idx in self._get_kmer_bin_indexes_bc(bin_seq):
