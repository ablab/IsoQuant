...
class PolyACounter(AbstractCounter):
    ...
    def add_read_info(self, read_assignment: ReadAssignment):
        ...
        polya_pos = -1
        if read_assignment.polyA_found and read_assignment.assignment_type.is_intronic_consistent():
    ...
    def dump(self):
        ...
        # FIXME replace 10 with self.DISTANCE_DELTA ?
        ...
        self.df['peak_count'] = self.df['histogram'].apply(lambda x: len(find_peaks(x, distance=self.DISTANCE_DELTA)[0]))
        self.df['peak_info'] = self.df['histogram'].apply(lambda x: find_peaks(x, distance=self.DISTANCE_DELTA, threshold=(None, None), height=(None, None))[1])
        self.df['peak_location'] = self.df['histogram'].apply(lambda x: [int(j - self.DISTANCE_DELTA) for j in find_peaks(x, distance=self.DISTANCE_DELTA)[0]])
        self.df['peak_prominence'] = self.df['histogram'].apply(lambda x: peak_prominences(x, find_peaks(x, distance=self.DISTANCE_DELTA)[0])[0])
        self.df['peak_width'] = self.df['histogram'].apply(lambda x: peak_widths(x, find_peaks(x, distance=self.DISTANCE_DELTA)[0], rel_height=0.98)[0])
        self.df['peak_left'] = self.df['histogram'].apply(lambda x: [int(j - self.DISTANCE_DELTA) for j in peak_widths(x, find_peaks(x, distance=self.DISTANCE_DELTA)[0], rel_height=self.RELATIVE_HEIGHT)[2]])
        self.df['peak_right'] = self.df['histogram'].apply(lambda x: [int(j - self.DISTANCE_DELTA) for j in peak_widths(x, find_peaks(x, distance=self.DISTANCE_DELTA)[0], rel_height=self.RELATIVE_HEIGHT)[3]])
        ...
        if 0 in list(self.df['peak_count']):
            ...
            self.dfResult['peak_heights'] = self.dfResult.apply(lambda x: x.histogram[x.prediction+self.DISTANCE_DELTA], axis = 1)
        ...
        # FIXME files can be printed with header, it will be resolved while merging
        if self.first:
