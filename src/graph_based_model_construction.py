    ...
    def construct_assignment_based_isoforms(self, read_assignment_storage):
        ...
        for read_assignment in read_assignment_storage:
            ...
            if any(e.event_type == MatchEventSubtype.mono_exon_match for e in events):
                ...
                if self.params.requires_polya_for_construction and self.gene_info.isoform_strands[reference_isoform_id] == '-':
                    if any(x.event_type == MatchEventSubtype.correct_polya_site_left for x in events):
                        isoform_left_support[reference_isoform_id] += 1
                elif abs(self.gene_info.all_isoforms_exons[reference_isoform_id][0][0] - read_assignment.corrected_exons[0][0]) <= self.params.apa_delta:
                    isoform_left_support[reference_isoform_id] += 1

                if self.params.requires_polya_for_construction and self.gene_info.isoform_strands[reference_isoform_id] == '+':
                    if any(x.event_type == MatchEventSubtype.correct_polya_site_right for x in events):
                        isoform_right_support[reference_isoform_id] += 1
                elif abs(self.gene_info.all_isoforms_exons[reference_isoform_id][-1][1] - read_assignment.corrected_exons[-1][1]) <= self.params.apa_delta:
                    isoform_right_support[reference_isoform_id] += 1
