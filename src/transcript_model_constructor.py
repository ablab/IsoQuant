...
class TranscriptModelConstructor:
    ...
    def blend_read_into_isoform(self, isoform_id, read_assignment):
        ...
        if SupplementaryMatchConstansts.extra_left_mod_position in modification_events_map:
            ...
            current_exon_start = self.process_intron_related_events(events, None, isoform_introns, read_introns,
                                                                    read_region, novel_exons, current_exon_start, isoform_id)
        else:
            current_exon_start = read_start
    ...
    def process_single_event(self, event_tuple, isoform_pos, isoform_introns, read_introns, read_region,
        ...
        if event_tuple.read_region[0] == SupplementaryMatchConstansts.absent_position:
            logger.warning("Undefined read intron position for event type: %s, isoform id %s" %
                           (event_tuple.event_type.name, isoform_id))
            return current_exon_start

        if not contains(read_region,
                        (read_introns[event_tuple.read_region[0]][0], read_introns[event_tuple.read_region[1]][1])):
            logger.warning("Read intron to be added seems to be outside of read region: %s, read: %s, intron: %s, isoform id %s" %
                           (event_tuple.event_type.name, str(read_region), str(read_introns), isoform_id))
            return current_exon_start
    ...
    def get_read_region(self, isoform_id, read_assignment):
        ...
        for e in isoform_match.match_subclassifications:
            if e.event_type in [MatchEventSubtype.correct_polya_site_right,
                                ...
                                MatchEventSubtype.internal_polya_right]:
